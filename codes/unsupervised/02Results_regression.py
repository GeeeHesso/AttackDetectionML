...
fig.tight_layout()
fig.savefig(
    pjoin("figures", "unsupervised_results_regression", f"{case}_learning.pdf"), dpi=600
)
plt.close(fig)
...
for metric in ["r²", "rmse", "normalized_rmse"]:
    for param in ["ds_type", "sequence", "contextual"]:
        ...
        if metric == "r²":
            ax.set(xlabel=metric)
        if metric == "rmse":
            ax.set(xlabel="RMSE [MW]")
        if metric == "normalized_rmse":
            ax.set(xlabel="Relative error", xlim=0)
        if param == "sequence":
            ax.set(ylabel="History length")
        if param == "contextual":
            ax.set(ylabel="Context")
        if param == "ds_type":
            ax.set(ylabel="Injections")
        if metric == "normalized_rmse":
            ax.set(xlim=(0, 0.15))
        fig.tight_layout()
        ...
        fig.savefig(
            pjoin(
                "figures",
                "unsupervised_results_regression",
                f"{case}_mlpr_{param}_{metric_name}.pdf",
            ),
            dpi=600,
        )
        plt.close(fig)
...
for metric in ["r²", "rmse", "normalized_rmse"]:
    ...
    # ax.set(xlabel=metric)
    if metric == "r²":
        ax.set(xlabel=metric, xlim=(0.8, 1), title="MLPR")
    if metric == "rmse":
        ax.set(xlabel="RMSE [MW]", xlim=0, title="MLPR")
    if metric == "normalized_rmse":
        ax.set(xlabel="Relative error", xlim=0, title="MLPR")
    fig.tight_layout()
    ...
    for param in ["ds_type"]:
        ...
        fig.savefig(
            pjoin(
                "figures",
                "unsupervised_results_regression",
                f"{case}_lstm_{param}_{metric_name}.pdf",
            ),
            dpi=600,
        )
        plt.close(fig)
...
for metric in ["r²", "rmse", "normalized_rmse"]:
    ...
    # ax.set(title=metric)
    if metric == "r²":
        ax.set(xlabel=metric, xlim=0)
    if metric == "rmse":
        ax.set(xlabel="RMSE [MW]", xlim=0)
    if metric == "normalized_rmse":
        ax.set(xlabel="Relative error", xlim=0)
    ax.set(title="LSTMR")
    fig.tight_layout()
        ...
        if set_type == "validation set":
            ...
            fig.savefig(
                pjoin(
                    "figures",
                    "unsupervised_results_regression",
                    f"{case}_mlpr-lstm_{metric_name}.pdf",
                ),
                dpi=600,
            )
            plt.close(fig)
