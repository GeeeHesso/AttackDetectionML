...
cartesian = product(nets_dict, models_dict.keys(), types_dict,
                    sequence_lens, contextuals_lens)
...
net_key_old = ''
c = 0  # Fit counter
cartesian_length = len(nets_dict) * len(models_dict.keys()) * len(types_dict) *\
                   len(sequence_lens) * len(contextuals_lens) 
for net_key, model_key, ds_type, seq, contextual in cartesian:
    ...
    if net_key != net_key_old:  # Avoid doing same things
        ...
        classification_test_index = rpckl(pjoin(dir_dataset, 'test_timesteps.p'))[0].to_list()
        # regression_index = [ts for ts in gen_p.index if ts not in classification_test_index]
        regression_validation_index = rpckl(pjoin(dir_dataset, 'regression_validation_timesteps.p')).to_list()
    ...
    for attacked_gen in attacked_gens:
        ...
        if not os.path.exists(dir_res): continue  # LSTM NOT 6*4 seq or contectual_t
        ...
        mape_lstm['val'][ds_type][attacked_gen] = mape(y_val_low.y_val, y_val_low.y_val_predict)
        
        # Test
        y_test_low = y_test[y_test.y_test > low_power]
        mape_lstm['test'][ds_type][attacked_gen] = mape(y_test_low.y_test, y_test_low.y_test_predict)
        ...
        if make_figures:
            ...
            ax1.hist(error_val.error, bins=100, alpha=.6)
            ax1.set(
            ...
            ax2.hist(error_val.error_hacked, bins=100, alpha=.6, color=colors[1])
            ax2.set(
            ...
            fig.tight_layout()
            fig.savefig(pjoin('figures', 'unsupervised_classification',
                              f'{net_key}_{model_key}_{ds_type}_len{seq}_{contextual}_{attacked_gen}_error.pdf'), dpi=600)
            plt.close(fig)
        ...
        threshold_fit.iloc[0:1,:].to_pickle(pjoin(dir_res, 'confusion_validation_set.p'))
        ...
        y_val[['label', 'classification', 'mistakes']].to_pickle(pjoin(dir_res, 'mistakes_validation_set.p'))

        y_test['classification'] = y_test['absolute_error_hacked'] > best_threshold
        y_test['mistakes'] = y_test['label'] ^ y_test['classification']
        y_test[['label', 'classification', 'mistakes']].to_pickle(pjoin(dir_res, 'mistakes_test_set.p'))
        ...
        if make_figures:
            ...
            ax.set(ylabel='F\u2082', ylim=(0, 1), title=get_gen_names(case)[attacked_gen],
                    xlabel='Seuil [MW]', xlim=(0,threshold_max))

            fig.tight_layout()
            fig.savefig(pjoin('figures', 'unsupervised_classification',
                              f'{net_key}_{model_key}_{ds_type}_len{seq}_{contextual}_{attacked_gen}_threshold.pdf'), dpi=600)
            plt.close(fig)
        ...
        y_val_hacked = y_val_hacked[y_val_hacked.y_hacked == 0]  # Attaque de mise à l'arrêt 

        low_power = 0.2 * p_nom[attacked_gen]
        low_power_lstm['val'][ds_type][attacked_gen] = (y_val_hacked.y_val < low_power).sum()
        
        # Test
        y_test_hacked = y_test[y_test.label == True]
        y_test_hacked = y_test_hacked[y_test_hacked.y_hacked == 0]  # Attaque de mise à l'arrêt 

        # low_power = 0.2 * p_nom[attacked_gen]
        low_power_lstm['test'][ds_type][attacked_gen] = (y_test_hacked.y_test < low_power).sum()
        ...
        pd.DataFrame({
            ...
            'tn': cm[0], 'fp': cm[1],
            'fn': cm[2], 'tp': cm[3],
            }, index=[0]).to_pickle(pjoin(dir_res, 'confusion_test_set.p'))
...
ax.scatter(std_lstm_gen['gen_index'], std_lstm_gen['threshold/std'], label='Productions')
mean_all = (std_lstm_inj['threshold/std'].mean() + std_lstm_gen['threshold/std'].mean()) / 2
ax.axhline(mean_all, ls='--', c='k', label=f'Moyenne: {mean_all:.1f}' )
ax.set(ylabel='Seuil / écart type', ylim=(0, 2 * mean_all),)
ax.legend(ncol=3)
