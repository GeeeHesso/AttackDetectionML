...
01GSCV.py :     Grid search cv files for cluster computing
				in armasuisse 2024 paper unsupervised machine learning.
...
cartesian_length = len(nets_dict) * len(models_dict.keys()) * len(types_dict) *\
                   len(sequence_lens) * len(contextual_lens)
...
for net_key, model_key, ds_type, seq, contextual in cartesian:
    ...
    for n_gen, attack_gen in enumerate(attacked_gens):
        ...
        param_key = f'{ds_type}_24_{contextual}' if seq == 24 else f'{ds_type}_4_{contextual}'
        grid = GridSearchCV(estimator=model['estimator'](),
                            param_grid=model['parameters'][net_key][param_key],
                            cv=5, scoring='sc', verbose=0,
                            n_jobs=-1, refit=True)
        ...
        y_test_predict = y_scaler.inverse_transform(y_test_predict_scaled.reshape(-1, 1))
        ...
        pd.DataFrame({
            ...
            }, index=y_val.index).to_pickle(pjoin(dir_result, 'prediction_validation_set.p'))
        
        pd.DataFrame({
            'y_test': y_test.iloc[:, 0],
            'y_test_predict': y_test_predict.ravel(),
            }, index=y_test.index).to_pickle(pjoin(dir_result, 'prediction_test_set.p'))
