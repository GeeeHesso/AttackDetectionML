...
# ignore scikit-learn FutureWarning
import warnings
...
for net_key, model_key, ds_type, seq, contextual in cartesian:
    ...
    for i, attack_gen in enumerate(attacked_gens):
        ...
        # [3.3] TRAIN SIZE LOOP
        train_sizes_eff = []
        n_trains, hacked_train, time_predict, time_train, = [], [], [], []
        ...
        # [10] SAVE INFOS
        cm = np.array(cm)
        confusion_df = pd.DataFrame({'train_size': train_sizes_eff,
        ...
        compute_time = pd.DataFrame({'inputs': X_tr.shape[1],
