...
04Results_classification_unique.py :  
    Compile all regression results from 
						  01GSCV.py armasuisse 2024 paper.
...
# %% LOAD RESULTS DF
cartesian = product(nets, models.keys(), types,
                    sequences, contextuals)
...
for net, model, ds_type, seq, contextual in cartesian:
    ...
    for i, node in enumerate(nodes):
        ...
        if not os.path.isfile(pjoin(dir_result, 'confusion_validation_set.p')): continue  # LSTM NOT 6*4 seq or contectual_t
...
for model, result_model in result_net.groupby('model'):
    for metric in ['f2_score']:
        for param in ['ds_type', 'sequence', 'contextual']:
            ...
            fig.tight_layout()
            fig.savefig(pjoin('figures', 'unsupervised_results_classification',
                              f'{case}_{model}_{metric}_{param}.pdf'), dpi=600)
            plt.close(fig)
...
for model, result_model in result_net.groupby('model'):
    for metric in ['f2_score']:
        ...
        fig.tight_layout()
        fig.savefig(pjoin('figures', 'unsupervised_results_classification',
                          f'{case}_{model}_{metric}.pdf'), dpi=600)
        plt.close(fig)
...
fig.tight_layout()
fig.savefig(pjoin('figures', 'unsupervised_results_classification', f'{case}_lstmr_prod-inj_boxplot.pdf'), dpi=600)
plt.close(fig)
...
ax.tick_params(axis='x', labelrotation=20)
fig.tight_layout()
fig.savefig(pjoin('figures', 'unsupervised_results_classification', f'{case}_lstmr_false_negatives.pdf'), dpi=600)
plt.close(fig)
...
ax.tick_params(axis='x', labelrotation=20)
fig.tight_layout()
fig.savefig(pjoin('figures', 'unsupervised_results_classification', f'{case}_lstmr_false_positives.pdf'), dpi=600)
plt.close(fig)
...
fig.tight_layout()
fig.savefig(pjoin('figures', 'unsupervised_results_classification', f'{case}_lstmr_recall.pdf'), dpi=600)
plt.close(fig)
...
fig, ax = plt.subplots(figsize=(8,2.5))  # precision (positive predictive) by node & type
...
for i, label in enumerate(ax.get_xticklabels()):
    label.set_y(label.get_position()[1] - (i % 2) * 0.075)
fig.tight_layout()
fig.savefig(pjoin('figures', 'unsupervised_results_classification', f'{case}_lstmr_precision.pdf'), dpi=600)
plt.close(fig)
